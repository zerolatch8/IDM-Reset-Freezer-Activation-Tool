...
::============================================================================
::
::   IDM Configuration Script (By ZeroLatch | Credits: ZIEDEV)
::
::   Homepage: https://github.com/zerolatch8/IDM-Reset-Freezer-Activation-Tool
::
::============================================================================



::  To configure, run the script with "/act" parameter or change 0 to 1 in below line
set _activate=0

::  To Preserve the 30 days evaluation period, run the script with "/frz" parameter or change 0 to 1 in below line
set _freeze=0

::  To reset the Configuration and trial, run the script with "/res" parameter or change 0 to 1 in below line
set _reset=0
...
set "blank="
set "mas=https://github.com/zerolatch8/IDM-Reset-Freezer-Activation-Tool"
...
echo:
echo:
echo Help - %mas%ZEROLATCH-Help#troubleshoot
echo:
echo:
ping 127.0.0.1 -n 10
)
...
cls
color 07
title  IDM Configuration Script %ZEROLATCH%
...
echo:
echo Check this page for help. %mas%ZEROLATCH-Help#troubleshoot
goto done2
)
...
%nul% reg delete HKCU\ZEROLATCH_TEST /f
%nul% reg delete HKU\%_sid%\ZEROLATCH_TEST /f

set HKCUsync=$null
%nul% reg add HKCU\ZEROLATCH_TEST
%nul% reg query HKU\%_sid%\ZEROLATCH_TEST && (
set HKCUsync=1
)
...
%nul% reg add %CLSID2%\ZEROLATCH_TEST
%nul% reg query %CLSID2%\ZEROLATCH_TEST || (
%eline%
...
%nul% reg delete %CLSID2%\ZEROLATCH_TEST /f
...
echo:
echo:
call :_color2 %_White% "             " %_Green% "Developed by ZeroLatch (Credits: ZIEDEV)"
echo:            ___________________________________________________
echo:
echo:               Github: https://github.com/zerolatch8/IDM-Reset-Freezer-Activation-Tool
echo:            ___________________________________________________
echo:
echo:               [1] configure
echo:               [2] Preserve Evaluation
echo:               [3] Reset Configuration / Trial
...
if %_erl%==5 start https://github.com/zerolatch8/IDM-Reset-Freezer-Activation-Tool & goto MainMenu
...
%psc% "$ErrorActionPreference = 'Stop'; try { $response = Invoke-WebRequest -Uri 'https://raw.githubusercontent.com/zerolatch38/IDM-Preserver-Configuration-Tool/main/version.txt' -UseBasicParsing; $latestVersion = $response.Content.Trim(); Write-Output $latestVersion } catch { Write-Output 'Error' }" > "%temp%\idm_version.txt"
...
if "%ver%"=="%_latest_ver%" (
    ...
    echo:
    echo Visit https://github.com/zerolatch8/IDM-Reset-Freezer-Activation-Tool to download the latest version.
)
...
for /f "tokens=2*" %%a in ('reg query "HKU\%_sid%\Software\DownloadManager" /v Serial %nul6%') do (
    call :_color %Green% "IDM is registered with a registration token."
    echo Token: %%b
...
echo %line%
echo:
echo      Configuration is not working for some users and IDM may show fake registration notice screen.
echo:
call :_color2 %_White% "     " %_Green% "Its recommended to use Preserve Evaluation option instead."
echo %line%
echo:
choice /C:19 /N /M ">    [1] Go Back [9] configure : "
...
echo:
if %frz%==0 (
call :_color %Green% "The IDM Configuration process has been completed."
echo:
call :_color %Gray% "If the invalid token screen appears, use the Preserve Evaluation option instead."
) else (
